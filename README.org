...
- [[https://github.com/dakra/ghostel][ghostel]] is a newly supported native backend infra option.
  ...
  - AI Code enables Ghostel image support for AI sessions:
    ...
    - the required =ultra-scroll= package dependency preserves partial-pixel
      scrolling for a tall local preview; it is dispatched only while that
      preview is visible, not as a global scrolling mode.  Set
      =ai-code-ghostel-image-preview-prefer-ultra-scroll= to =nil= to opt out.
