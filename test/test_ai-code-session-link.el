...
(ert-deftest ai-code-session-link-test-image-preview-max-dimensions ()
  "Preview caps: integer customs are hard caps; nil falls back without a window."
  ...
  (let ((ai-code-session-link-ghostel-image-preview-max-width 400)
    ...
    (with-temp-buffer
      (should (equal (ai-code-session-link--image-preview-max-dimensions "")
                     '(400 . 300)))))
  ;; With nil customs and no window showing the buffer, the fixed fallback
  ;; applies (a temp buffer is never displayed).
