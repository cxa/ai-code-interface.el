...
When nil, previews fit the session window's body height.  Previews only ever
scale down, so small images keep their native size either way.  Set an
integer to impose a fixed hard cap instead."
  :type '(choice (const :tag "Fit session window" nil)
                 (integer :tag "Fixed pixel cap"))
...
(defconst ai-code-session-link--image-preview-fallback-max-height 640
  "Height cap used when no window is available to fit the preview to.")
...
is nil the preview fits the session window's body -- its width reduced by
INDENT so the image never overflows into a horizontal scroll.  These are
upper bounds only: `create-image' scales large images down to fit but never
enlarges a small one.  A fixed fallback applies when no window shows the
buffer."
  (let* ((window (get-buffer-window (current-buffer) t))
    ...
    (cons
     ...
     (cond
      ((integerp ai-code-session-link-ghostel-image-preview-max-height)
       ai-code-session-link-ghostel-image-preview-max-height)
      (window (max 1 (window-body-height window t)))
      (t ai-code-session-link--image-preview-fallback-max-height)))))
...
(defun ai-code-session-link--apply-image-preview-for-file
  ...
  (when (ai-code-session-link--image-preview-enabled-p)
    (let* ((indent (ai-code-session-link--image-preview-indent match-start))
      ...
      (when image
        (let ((overlay (make-overlay match-start match-end nil nil nil)))
...
(defun ai-code-session-link--strict-previous-line-prefix (position)
  "Return a safe path prefix from the line before POSITION, or nil."
  (save-excursion
    (goto-char position)
    (unless (bobp)
      (forward-line -1)
      (let* ((line
        ...
        (when line
          (cond
           ...
           ;; A negated class must list `]' first to treat it literally;
           ;; the previous `\\[\\]' spelling closed the class early, so it
           ;; only matched lines ending in a literal `]' and never recovered
           ;; ordinary wrapped paths.
           ((string-match "/[^] \t\n\r\"'`<>(){}[]+\\'" line)
            (match-string 0 line))))))))
          ...
          (while (and (< candidate-count
            ...
            (let* ((bounds
              ...
              (when link-text
                (catch 'previewed
                  (dolist (candidate
                           (ai-code-session-link--strict-image-candidate-texts
                            match-start link-text))
                    (when-let* ((file
                      ...
                      (ai-code-session-link--apply-properties
                       ...
                       "mouse-1: Visit image file")
                      (ai-code-session-link--apply-image-preview-for-file
                       match-start match-end candidate file link-text)
                      (throw 'previewed t))))))))))))
